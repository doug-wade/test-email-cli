...
module.exports = function ({ argv, config, dateProvider, logger, persister, sub }) {
  return new Promise((resolve, reject) => {
    const opts = { argv, config, dateProvider, logger, persister, sub };
...
function runCommand({ command, opts, sub }) {
  return new Promise((resolve, reject) => {
    ...
    if (command === 'init') {
      ...
      opts.logger.error('There is a problem with your config; running test-email init to set up config');
